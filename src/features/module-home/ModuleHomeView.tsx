...
export const ModuleHomeView: React.FC<ModuleHomeViewProps> = ({ moduleId }) => {
  ...
  const { moduleProgressMap, loadModuleProgress, recentSessions, loadRecentSessions } =
    useProgressStore();
  ...
  return (
    <div className="min-h-screen pb-24">
      ...
      <PageContainer maxWidth="md" className="space-y-6">
            ...
            {progress?.bestTimeMs && (
              <div className="text-right">
                ...
                <span className="font-mono font-black text-xl text-white">
                  {formatDuration(progress.bestTimeMs)}
                </span>
              </div>
            )}
          </div>
        ...
        {/* Primary Actions: HỌC TẬP vs LUYỆN TẬP (Section 11) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Button
            size="lg"
            variant="secondary"
            leftIcon={<BookOpen className="w-5 h-5 text-indigo-400" />}
            onClick={() => navigate({ name: 'learning', moduleId })}
          ...
          <Button
            ...
            onClick={() => navigate({ name: 'training-setup', moduleId })}
            className="h-16 text-base"
          >
            LUYỆN TẬP
          </Button>
        ...
        {/* Recent Performance History (Section 11) */}
        <div className="bg-slate-900/60 border border-slate-800/80 rounded-3xl p-5 sm:p-6 space-y-4">
          ...
          {moduleSessions.length === 0 ? (
            <div className="text-center py-6 text-slate-500 text-xs sm:text-sm">
              Chưa có phiên luyện tập nào. Hãy bấm <strong>LUYỆN TẬP</strong> để bắt đầu thử thách!
            </div>
          ) : (
            <div className="space-y-2.5">
              {moduleSessions.slice(0, 5).map((session) => (
                <div
                  key={session.id}
                  className="flex items-center justify-between p-3.5 rounded-2xl bg-slate-950/60 border border-slate-800/60 text-xs"
                >
                    ...
                    <div>
                      ...
                      <div className="text-[11px] text-slate-500 flex items-center gap-1.5 mt-0.5">
                        ...
                        {session.averageReactionMs && (
                          <>
                            <span>•</span>
                            <span>{formatReactionSpeed(session.averageReactionMs)}</span>
                          </>
                        )}
                      </div>
                    </div>
