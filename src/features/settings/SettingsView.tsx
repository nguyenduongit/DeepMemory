...
export const SettingsView: React.FC = () => {
  ...
  return (
    <PageContainer maxWidth="md" className="pb-24 space-y-6">
      ...
      {/* Preferences Section (Section 82) */}
      <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-5 sm:p-6 space-y-4">
        ...
        <div className="flex items-center justify-between p-3 rounded-2xl bg-slate-950/60 border border-slate-800/80">
          ...
          <button
            ...
            aria-label="Chuyển đổi hiệu ứng chuyển động"
            className={`w-12 h-6 flex items-center rounded-full p-1 transition-colors ${
              settings.reducedMotion ? 'bg-indigo-600' : 'bg-slate-800'
            }`}
          >
            <div
              className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform ${
      ...
      <div className="bg-slate-900/80 border border-rose-900/40 rounded-3xl p-5 sm:p-6 space-y-3">
        <h3 className="font-display font-bold text-sm text-rose-400 uppercase tracking-wider flex items-center gap-1.5">
          <ShieldAlert className="w-4 h-4" /> Vùng dữ liệu
        </h3>
        <p className="text-xs text-slate-400 leading-relaxed">
          Toàn bộ dữ liệu luyện tập, mức độ thành thạo và kỷ lục thời gian được lưu trữ cục bộ
          trên thiết bị của bạn qua IndexedDB.
        </p>

        {resetSuccess && (
          <div className="p-3 rounded-xl bg-emerald-950/30 border border-emerald-800/50 text-emerald-400 text-xs flex items-center gap-2">
            <Check className="w-4 h-4" /> Đã đặt lại dữ liệu thành công!
          </div>
        )}
        ...
        {!confirmReset ? (
          ...
          <div className="p-4 rounded-2xl bg-rose-950/30 border border-rose-800/60 space-y-3">
            <p className="text-xs font-semibold text-rose-300">
              Bạn có chắc chắn muốn xóa toàn bộ lịch sử luyện tập và điểm kỷ lục? Thao tác này không thể hoàn tác.
            </p>
            <div className="flex items-center gap-3">
      ...
      <div className="p-5 rounded-3xl bg-slate-950/60 border border-slate-800/80 text-center space-y-2">
        <div className="inline-flex items-center gap-1 text-xs text-indigo-400 font-bold uppercase font-mono">
          <Info className="w-3.5 h-3.5" /> AIO Memory Platform
        </div>
        <p className="text-[11px] text-slate-500 font-mono">
          Phiên bản 1.0.0 (MVP) • Universal Engine Architecture
        </p>
      </div>
    </PageContainer>
  );
