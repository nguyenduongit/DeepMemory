...
export const StatisticsView: React.FC = () => {
  const { moduleProgressMap, itemProgressMap, loadAllModuleData } =
    useProgressStore();
  ...
  const summary = calculateModuleMasterySummary(
    numbersModule.items.length,
    itemsProgress
  );
  ...
  return (
    <PageContainer maxWidth="md" className="pb-24 space-y-6">
      ...
      {/* Overview Stat Cards (Section 37) */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4">
          <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">
            Thành thạo
          </span>
          <span className="font-display font-black text-2xl sm:text-3xl text-indigo-400 mt-1 block">
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4">
          ...
          <span className="font-display font-black text-2xl sm:text-3xl text-emerald-400 mt-1 block">
            {accuracy}%
          </span>
        </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4">
          ...
          <span className="font-display font-black text-2xl sm:text-3xl text-white mt-1 block">
            {totalAnswers}
          </span>
        </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4">
          ...
          <span className="font-display font-black text-2xl sm:text-3xl text-amber-400 mt-1 block">
            {numProgress?.completedSessions || 0}
          </span>
        </div>
      </div>

      {/* Mastery Breakdown Distribution (Section 38, 40) */}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="p-3.5 rounded-2xl bg-emerald-950/20 border border-emerald-900/40">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-emerald-400 uppercase">Mastered</span>
              <span className="font-mono font-bold text-lg text-white">
                {summary.levelCounts.mastered}
              </span>
            </div>
          ...
          <div className="p-3.5 rounded-2xl bg-cyan-950/20 border border-cyan-900/40">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-cyan-400 uppercase">Familiar</span>
              <span className="font-mono font-bold text-lg text-white">
                {summary.levelCounts.familiar}
              </span>
            </div>
          ...
          <div className="p-3.5 rounded-2xl bg-amber-950/20 border border-amber-900/40">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-amber-400 uppercase">Learning</span>
              <span className="font-mono font-bold text-lg text-white">
                {summary.levelCounts.learning}
              </span>
            </div>
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-5 sm:p-6 space-y-4">
        ...
        {weakItems.length === 0 ? (
          <p className="text-xs text-slate-500 py-3 text-center">
            Tuyệt vời! Hiện chưa có câu nào bị sai nhiều lần.
          </p>
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <span className="font-mono font-black text-white text-base bg-slate-900 px-2 py-1 rounded-lg border border-slate-800">
                      {numItem?.number || item.itemId}
                    </span>
                    <div>
                      ...
                      <span className="text-[11px] text-rose-400">
                        Sai {item.wrongCount} lần ({item.accuracy}%)
                      </span>
                    </div>
                  </div>
